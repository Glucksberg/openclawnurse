...
build_openclaw_cmd() {
  ...
  if [[ -n "${OPENCLAW_NODE_BIN:-}" && -n "${OPENCLAW_NODE_ENTRYPOINT:-}" ]]; then
    local node_dir
    node_dir="${OPENCLAW_NODE_PATH_DIR:-$(dirname "$OPENCLAW_NODE_BIN")}"
...
OPENCLAW_NODE_TARGET_ENGINE_RANGE=""
...
openclaw_package_root_from_ancestors() {
  local path="$1"
  local current
  ...
  while [[ -n "$current" ]]; do
    if [[ -f "$current/package.json" && "$(jq -r '.name // empty' "$current/package.json" 2>/dev/null)" == "openclaw" ]]; then
      printf '%s\n' "$current"
      return 0
    fi
    [[ "$current" == "/" ]] && break
...
openclaw_package_root_from_path() {
  local path="$1"
  local real root embedded
  real="$(canonical_path "$path")"

  root="$(openclaw_package_root_from_ancestors "$real" || true)"
  if [[ -n "$root" ]]; then
    printf '%s\n' "$root"
    return 0
  fi
  ...
  if [[ -f "$real" ]]; then
    ...
    while IFS= read -r embedded; do
      [[ -n "$embedded" && "$embedded" != "$real" ]] || continue
      root="$(openclaw_package_root_from_ancestors "$embedded" || true)"
      if [[ -n "$root" ]]; then
        printf '%s\n' "$root"
        return 0
      fi
    done < <(grep -oE "/[^[:space:]\"';]+" "$real" 2>/dev/null || true)
  fi
  return 1
}
...
gateway_systemd_runtime_tokens() {
  [[ "$RESTART_MODE" == "systemd_user" && -n "$SYSTEMD_UNIT_NAME" ]] || return 1
  command_exists "$SYSTEMCTL_BIN" || return 1

  local pid token exec_start
  pid="$(run_runtime_probe "$SYSTEMCTL_BIN" --user show "$SYSTEMD_UNIT_NAME" -p MainPID --value 2>/dev/null || true)"
  if [[ "$pid" =~ ^[0-9]+$ && "$pid" -gt 0 && -r "$PROCFS_DIR/$pid/cmdline" ]]; then
    while IFS= read -r token; do
      [[ -n "$token" ]] && printf '%s\n' "$token"
    done < <(tr '\0' '\n' < "$PROCFS_DIR/$pid/cmdline")
  fi
...
active_openclaw_package_root() {
  ...
  done < <(gateway_systemd_runtime_tokens || true)
  return 1
}
...
refresh_selected_openclaw_entrypoint() {
  [[ -n "$OPENCLAW_NODE_BIN" ]] || return 0
...
openclaw_node_path_dir() {
  ...
  node_dir="$(dirname "$node_bin")"
  if [[ "$(basename "$node_bin")" == "node" ]]; then
    printf '%s\n' "$node_dir"
    return 0
  fi

  shim_key="$(printf '%s' "$node_bin" | cksum | awk '{print $1}')"
  shim_dir="$STATE_DIR/openclaw-node-runtime-bin/$shim_key"
  shim_target="$(canonical_path "$node_bin")"
  mkdir -p "$shim_dir" || return 1
  ln -sfn -- "$shim_target" "$shim_dir/node" || return 1
  printf '%s\n' "$shim_dir"
}
...
select_compatible_openclaw_node() {
  ...
  for candidate in "${candidates[@]}"; do
    ...
    node_dir="$(openclaw_node_path_dir "$candidate" || true)"
    [[ -n "$node_dir" ]] || continue
    ...
    OPENCLAW_NODE_PATH_DIR="$node_dir"
    return 0
  done
  return 1
}
...
read_target_openclaw_package_metadata() {
  local root target npm_bin npm_dir output status version range candidate
  ...
  for npm_bin in "${npm_candidates[@]}"; do
    ...
    output="$(PATH="$npm_dir:$PATH" run_runtime_probe "$npm_bin" view "openclaw@$target" version --json 2>/dev/null)"
    status=$?
    [[ "$status" -eq 0 ]] || continue
    version="$(printf '%s' "$output" | jq -r 'if type == "string" then . elif type == "array" then last // empty else .version // empty end' 2>/dev/null || true)"
    [[ "$version" =~ ^[0-9]{4}\.[0-9]+\.[0-9]+([-+][0-9A-Za-z.-]+)?$ ]] || continue

    output="$(PATH="$npm_dir:$PATH" run_runtime_probe "$npm_bin" view "openclaw@$version" engines.node --json 2>/dev/null)"
    status=$?
    [[ "$status" -eq 0 ]] || continue
    range="$(printf '%s' "$output" | jq -r 'if type == "string" then . elif type == "object" then .node // empty else empty end' 2>/dev/null || true)"
...
prepare_openclaw_node_runtime_for_update() {
  ...
  local metadata target_version engine_range current_node
  metadata="$(read_target_openclaw_package_metadata || true)"
  target_version="$(printf '%s\n' "$metadata" | sed -n '1p')"
  engine_range="$(printf '%s\n' "$metadata" | sed -n '2p')"
  if [[ -z "$target_version" || -z "$engine_range" ]]; then
    add_incident_code "openclaw_node_runtime_preflight_failed"
    append_array ERRORS "Could not determine the target OpenClaw version and Node engine; the update was blocked before package files were changed."
    append_array ACTIONS "Verify npm registry access and the target OpenClaw package metadata, then retry the Nurse run."
    record_remediation "openclaw_node_runtime" "blocked_engine_metadata_unavailable" "target version and Node engine could not be read"
    return 1
  fi
...
recover_openclaw_node_runtime_after_failure() {
  ...
  local detail
  if [[ "$node_failure" -eq 1 ]]; then
    detail="$(remediation_detail_from_output "$failure_output")"
  else
    detail="$(remediation_detail_from_output "$cli_output")"
  fi
  if [[ "$AUTO_SELECT_COMPATIBLE_OPENCLAW_NODE" != "true" ]]; then
    add_incident_code "openclaw_node_runtime_recovery_blocked"
    append_array ERRORS "The OpenClaw update requires runtime recovery, but automatic compatible-Node selection is disabled."
    append_array ACTIONS "Enable AUTO_SELECT_COMPATIBLE_OPENCLAW_NODE or recover the OpenClaw runtime manually before retrying the update."
    record_remediation "openclaw_node_runtime" "recovery_blocked_selection_disabled" "$detail"
    return 2
  fi

  local root installed_range engine_range
  local engine_ranges=()
  root="$(active_openclaw_package_root || true)"
  if [[ -n "$root" && -f "$root/package.json" ]]; then
    installed_range="$(jq -r '.engines.node // empty' "$root/package.json" 2>/dev/null || true)"
  fi
  if [[ -n "$OPENCLAW_NODE_TARGET_ENGINE_RANGE" ]]; then
    append_unique_array engine_ranges "$OPENCLAW_NODE_TARGET_ENGINE_RANGE"
  elif [[ -n "$installed_range" ]]; then
    append_unique_array engine_ranges "$installed_range"
  elif [[ -n "$OPENCLAW_NODE_ENGINE_RANGE" ]]; then
...
build_openclaw_npm_cmd() {
  ...
  # shellcheck disable=SC2178 # nameref points at an array selected by name.
  local -n ref="$name"
  local npm_bin node_dir candidate_dir

  if [[ -n "$OPENCLAW_NODE_BIN" ]]; then
    node_dir="${OPENCLAW_NODE_PATH_DIR:-$(dirname "$OPENCLAW_NODE_BIN")}"
    candidate_dir="$(dirname "$OPENCLAW_NODE_BIN")"
    npm_bin="$candidate_dir/npm"
    if [[ -x "$npm_bin" ]]; then
...
run_preflight_checks() {
  ...
  local required=(jq flock timeout)
  local cmd
  ...
  if [[ "$RESTART_MODE" == "systemd_user" ]]; then
    append_unique_array required "$SYSTEMCTL_BIN"
  fi
...
build_openclaw_update_cmd() {
  ...
  if [[ -n "$OPENCLAW_UPDATE_TARGET_VERSION" ]]; then
    ref+=(--tag "$OPENCLAW_UPDATE_TARGET_VERSION")
...
validate_openclaw_cli_after_update() {
  local output status version recovery_status=0 detail=""

  refresh_selected_openclaw_entrypoint || true
  output="$(openclaw_cli_version 2>&1)"
  status=$?
  version="$(printf '%s' "$output" | extract_openclaw_version)"
  if [[ "$status" -eq 0 && -n "$version" ]]; then
    if [[ -z "$OPENCLAW_UPDATE_TARGET_VERSION" || "$version" == "$OPENCLAW_UPDATE_TARGET_VERSION" ]]; then
      CURRENT_VERSION_AFTER="$version"
      return 0
    fi
    detail="OpenClaw reported version $version after updating to expected version $OPENCLAW_UPDATE_TARGET_VERSION"
  fi
  ...
  [[ -n "$detail" ]] || detail="$(remediation_detail_from_output "$output")"
  [[ -n "$detail" ]] || detail="OpenClaw version probe failed with exit $status after the updater returned success"
  UPDATE_ERROR="$detail"
  GATEWAY_SERVICE_REFRESH_BLOCKS_RESTART=1
  add_incident_code "post_update_cli_validation_failed"
  append_array ERRORS "The OpenClaw updater returned success, but the installed CLI failed post-update validation."
  append_array ACTIONS "Repair or reinstall OpenClaw before restarting the gateway; automatic restart was skipped."
  record_remediation "openclaw_node_runtime" "post_update_validation_failed" "$detail"
  return 1
}
...
run_update() {
  ...
  if [[ "$status" -eq 0 ]]; then
    if ! validate_openclaw_cli_after_update; then
      UPDATE_SUCCEEDED=0
  ...
  if [[ "$recovery_status" -eq 2 ]]; then
    CONSECUTIVE_FAILURES=$((CONSECUTIVE_FAILURES + 1))
    append_array ERRORS "OpenClaw update failed and the required runtime recovery could not be completed."
    classify_update_failure "$output"
    return 1
  fi
  ...
  if [[ "$RESTART_MODE" == "systemd_user" ]]; then
    ...
    if [[ -n "$OPENCLAW_NODE_BIN" ]]; then
      run_capture output status "Restarting gateway with custom command" \
        env \
          OPENCLAW_RUNTIME_NODE_DIR="${OPENCLAW_NODE_PATH_DIR:-$(dirname "$OPENCLAW_NODE_BIN")}" \
